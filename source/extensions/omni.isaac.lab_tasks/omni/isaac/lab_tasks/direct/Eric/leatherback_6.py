...
class LeatherbackEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: LeatherbackEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._target_positions = torch.zeros((self.num_envs, 10, 2), device=self.device, dtype=torch.float32)
        self._markers_pos = torch.zeros((self.num_envs, 10, 3), device=self.device, dtype=torch.float32)
    ...
    def _get_observations(self) -> dict:
        ...
        # position error
        self._position_error = self._target_positions[:, 0, :2] - self.Leatherback.data.root_pos_w[:, :2]
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # position reward
        self.position_exponential_reward_coeff: float = 1.0
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        self.goal_is_reached = (self._position_dist < position_tolerance).int()        
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self._target_positions[env_ids, 0, 0] = -0.8 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, 1, 0] = -0.6 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, 2, 0] = -0.4 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, 3, 0] = -0.2 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, 4, 0] = 0.0 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, 5, 0] = 0.2 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, 6, 0] = 0.4 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, 7, 0] = 0.6 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, 8, 0] = 0.8 * self.env_spacing / self.course_length_coefficient
        self._target_positions[env_ids, :, 1] = torch.rand((num_reset, 10), dtype=torch.float32, device=self.device) * self.course_width_coefficient
        self._target_positions[env_ids, :] += self.scene.env_origins[env_ids, :2].unsqueeze(1)

        # Update the visual markers
        self._markers_pos[env_ids, :, :2] = self._target_positions[env_ids]
        visualize_pos = self._markers_pos.view(-1, 3)
        self.Cones.visualize(translations=visualize_pos)
        #endregion Set Goals

        #region Make sure the position error and position dist are up to date after the reset
        # HACK: not sure how to handle these yet
        # self._position_error[env_ids] = (
        #     self._target_positions[env_ids] - self.Leatherback.data.root_pos_w[env_ids, :2][env_ids].unsqueeze(1)
        # )
